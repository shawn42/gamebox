...
class Object
  class << self
    ...
    # RubyGems, ~/.gamebox/generators, and GAMEBOX_ROOT/generators.
    def lookup_missing_generator(class_id)
      if md = /(.+)Generator$/.match(class_id.to_s)
        name = md.captures.first.demodulize.underscore
        Gamebox::Generator::Base.lookup(name).klass
      else
  ...
  module Generator
    ...
    module Lookup
      ...
      module ClassMethods
        ...
        # 1.  Gamebox application.  If GAMEBOX_ROOT is defined we know we're
        #     generating in the context of a Gamebox application, so search
        #     GAMEBOX_ROOT/generators.
        # 2.  Look in plugins, either for generators/ or gamebox_generators/ 
        #     directories within each plugin
        # 3.  User home directory.  Search ~/.gamebox/generators.
        # 4.  RubyGems.  Search for gems named *_generator, and look for 
        #     generators within any RubyGem's 
        #     /gamebox_generators/<generator_name>_generator.rb file.
        # 5.  Builtins.  Model, controller, mailer, scaffold, and so on.
        def use_component_sources!
          reset_sources
          if defined? ::GAMEBOX_ROOT
            sources << PathSource.new(:lib, "#{::GAMEBOX_ROOT}/lib/generators")
            sources << PathSource.new(:vendor, "#{::GAMEBOX_ROOT}/vendor/generators")
            Gamebox.configuration.plugin_paths.each do |path|
              relative_path = Pathname.new(File.expand_path(path)).relative_path_from(Pathname.new(::GAMEBOX_ROOT))
              sources << PathSource.new(:"plugins (#{relative_path})", "#{path}/*/**/{,gamebox_}generators")
            end
          end
          sources << PathSource.new(:user, "#{Dir.user_home}/.gamebox/generators")
    ...
    # GemPathSource looks for generators within any RubyGem's /gamebox_generators/<generator_name>_generator.rb file.
    class GemPathSource < AbstractGemSource
      # Yield each generator within gamebox_generator subdirectories.
      def each
        ...
        def generator_full_paths
          @generator_full_paths ||=
            Gem::cache.inject({}) do |latest, name_gem|
              ...
              Dir[gem.full_gem_path + '/{gamebox_,}generators/**/*_generator.rb'].each do |generator|
                mem << generator
              end
              mem
            end
