    ...
    module Scripts
      ...
      class Base
        ...
        def run(args = [], runtime_options = {})
          ...
          Gamebox::Generator::Base.instance(options[:generator], args, options).command(options[:command]).invoke!
        rescue => e
          puts e
        ...
        protected
          ...
          def usage_message
            ...
            Gamebox::Generator::Base.sources.inject([]) do |mem, source|
...
More are available at http://wiki.rubyonrails.org/rails/pages/AvailableGenerators
  ...
  2. Unzip to directory #{Dir.user_home}/.gamebox/generators/login
     to use the generator with all your Gamebox apps
end_blurb

            if Object.const_defined?(:GAMEBOX_ROOT)
              usage << <<end_blurb
     or to #{File.expand_path(GAMEBOX_ROOT)}/lib/generators/login
