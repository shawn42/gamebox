...
module Gamebox
  class TemplateRunner
    ...
    # Create a new file in the Gamebox project folder.  Specify the
    # relative path from gamebox_ROOT.  Data is the return value of a block
    # or a data string.
    #
    # ==== Examples
    #
    ...
    def environment(data = nil, &block)
      sentinel = 'Gamebox::Initializer.run do |config|'
    ...
    # Generate something using a generator from Gamebox or a plugin.
    ...
    def rake(command, options = {})
      ...
      in_root { run("#{sudo}rake #{command} gamebox_ENV=#{env}", false) }
    end
    ...
    #
    def freeze!(args = {})
      log 'vendor', 'gamebox edge'
      in_root { run('rake gamebox:freeze:edge', false) }
    end

    # Make an entry in Gamebox routing file conifg/routes.rb
    #
    # === Example
    #
    ...
    # ==== Example
    #
    #   answer = ask("Should I freeze the latest Gamebox?")
    #   freeze! if ask("Should I freeze the latest Gamebox?") == "yes"
    #
    def ask(string)
      log '', string
      gets.strip
    ...
    # Do something in the root of the Gamebox application or
    ...
    # ==== Example
    #
    #   freeze! if yes?("Should I freeze the latest Gamebox?")
    #
    def yes?(question)
    ...
    def logger
      @logger ||= Gamebox::Generator::Base.logger
    end

    def logger
      @logger ||= if defined?(Gamebox::Generator::Base)
        Gamebox::Generator::Base.logger
      else
        require 'gamebox_generator/simple_logger'
        Gamebox::Generator::SimpleLogger.new(STDOUT)
      end
    end
