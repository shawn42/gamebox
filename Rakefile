...
require File.join(File.dirname(__FILE__), 'lib/gamebox', 'version')

PKG_BUILD       = ENV['PKG_BUILD'] ? '.' + ENV['PKG_BUILD'] : ''
PKG_NAME        = 'gamebox'
PKG_VERSION     = Gamebox::VERSION::STRING + PKG_BUILD
PKG_FILE_NAME   = "#{PKG_NAME}-#{PKG_VERSION}"
PKG_DESTINATION = ENV["gamebox_PKG_DESTINATION"] || "../#{PKG_NAME}"

RELEASE_NAME  = "REL #{PKG_VERSION}"

RUBY_FORGE_PROJECT = "gamebox"
RUBY_FORGE_USER    = "shawn42"
...
HTML_FILES   = %w( 422.html 404.html 500.html index.html robots.txt favicon.ico images/gamebox.png
...
desc "Generates a fresh Gamebox package with documentation"
task :fresh_gamebox => [ :clean, :make_dir_structure, :initialize_file_stubs, :copy_vendor_libraries, :copy_ties_content, :generate_documentation ]

desc "Generates a fresh Gamebox package using GEMs with documentation"
task :fresh_gem_gamebox => [ :clean, :make_dir_structure, :initialize_file_stubs, :copy_ties_content, :copy_gem_environment ]

desc "Generates a fresh Gamebox package without documentation (faster)"
task :fresh_gamebox_without_docs => [ :clean, :make_dir_structure, :initialize_file_stubs, :copy_vendor_libraries, :copy_ties_content ]

desc "Generates a fresh Gamebox package without documentation (faster)"
task :fresh_gamebox_without_docs_using_links => [ :clean, :make_dir_structure, :initialize_file_stubs, :link_vendor_libraries, :copy_ties_content ]

desc "Generates minimal Gamebox package using symlinks"
task :dev => [ :clean, :make_dir_structure, :initialize_file_stubs, :link_vendor_libraries, :copy_ties_content ]

desc "Packages the fresh Gamebox package with documentation"
task :package => [ :clean, :fresh_gamebox ] do
...
desc "Make the directory structure for the new Gamebox application"
...
desc "Copy in all the Gamebox packages to vendor"
task :copy_vendor_libraries do
  mkdir File.join(PKG_DESTINATION, 'vendor', 'gamebox')
  VENDOR_LIBS.each { |dir| cp_r File.join('..', dir), File.join(PKG_DESTINATION, 'vendor', 'gamebox', dir) }
  FileUtils.rm_r(Dir.glob(File.join(PKG_DESTINATION, 'vendor', 'gamebox', "**", ".svn")))
end

desc "Link in all the Gamebox packages to vendor"
task :link_vendor_libraries do
  mkdir File.join(PKG_DESTINATION, 'vendor', 'gamebox')
  VENDOR_LIBS.each { |dir| ln_s File.join('..', '..', '..', dir), File.join(PKG_DESTINATION, 'vendor', 'gamebox', dir) }
end
...
task :copy_configs do
  ...
  cp "configs/initializers/new_gamebox_defaults.rb",  "#{PKG_DESTINATION}/config/initializers/new_gamebox_defaults.rb"
...
task :generate_documentation => [ :generate_app_doc, :generate_gamebox_framework_doc ]

task :generate_gamebox_framework_doc do
  system %{cd #{PKG_DESTINATION}; rake doc:gamebox}
end
...
Rake::RDocTask.new { |rdoc|
  rdoc.rdoc_dir = 'doc'
  rdoc.title    = "Railties -- Gluing the Engine to the Gamebox"
  rdoc.options << '--line-numbers' << '--inline-source' << '--accessor' << 'cattr_accessor=object'
  rdoc.options << '--charset' << 'utf-8'
  rdoc.template = ENV['template'] ? "#{ENV['template']}.rb" : '../doc/template/horo'
  rdoc.rdoc_files.include('README', 'CHANGELOG')
  rdoc.rdoc_files.include('lib/*.rb')
  rdoc.rdoc_files.include('lib/gamebox/*.rb')
  rdoc.rdoc_files.include('lib/gamebox_generator/*.rb')
...
spec = Gem::Specification.new do |s|
  ...
  s.description = <<-EOF
    Gamebox is a framework for building web-application using CGI, FCGI, mod_ruby, or WEBrick
  ...
  s.default_executable = "gamebox"

  s.author = "David Heinemeier Hansson"
  s.email = "david@loudthinking.com"
  s.homepage = "http://www.rubyongamebox.org"
